...
function applab_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'applab' ),
        'next_string'     => esc_html__( '&raquo;', 'applab' ),
        'before_output'   => '<div class="applab-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'applab_pagination_defaults', $defaults )
    );
