...
function applab_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'applab' ) . '</span></li>';
...
// Blog categories
function applab_featured_post_cat(){
...
// social media
if ( ! function_exists( 'applab_social' ) ) {
	function applab_social( $args = array() ) {
		...
		$url = applab_opt( 'applab_social_url' );
...
// header cart count
function applab_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( APPLAB_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'applab' ); ?>">
...
function applab_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'applab_contact7_form_content', 10, 2 );