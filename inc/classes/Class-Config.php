...
final class Applab {
	...
	private function setup() {
		...
		$enqueu          = new applab_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->applab_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'applab_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'applab', APPLAB_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'applab_site_logo_94x33', 94, 33, true );

		// Hero right thumb size
		add_image_size( 'applab_hero_right_thumb_464x680', 464, 680, true );

		// Service section thumb size
		add_image_size( 'applab_service_man_thumb_372x655', 372, 655, true );
		add_image_size( 'applab_service_right_thumb_561x707', 561, 707, true );

		// Feature section thumb size
		add_image_size( 'applab_feature_right_illustration_thumb_588x341', 588, 341, true );
		add_image_size( 'applab_feature_left_phone_thumb_336x680', 336, 680, true );

		// Latest post thumbnail size
		add_image_size( 'applab_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'applab_single_blog_750x375', 750, 375, true );
		add_image_size( 'applab_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'applab' ),
			'services'     => esc_html__( 'Services', 'applab' ),
			'useful-links' => esc_html__( 'Useful Links', 'applab' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = APPLAB_DIR_CSS_URI;
		$jsPath  = APPLAB_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->applab_version,
				),
				array(
					'handler' => 'applab-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'applab' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate applab theme customizer
		$applab_theme_customizer = new applab_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'applab_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'applab-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'applab_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'applab-elementor-default-style',  APPLAB_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->applab_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'applab_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'applab-elementor-notice', APPLAB_DIR_JS_URI.'applab-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'applab-elementor-notice',
				'applabElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'applab-elementor-notice-nonce' ),
				)
			);
		}
